...
import DataImage, { listProyek, listTimeline, listTools } from "../data";
...
const contactMethods = [
  {
    label: "Email",
    value: "dapakyla18@gmail.com",
    href: "mailto:dapakyla18@gmail.com",
    icon: "ri-mail-line",
    tone: "text-violet-200 border-violet-500/40 hover:bg-violet-500/10",
  },
  {
    label: "GitHub",
    value: "github.com/davfaa",
    href: "https://github.com/davfaa",
    icon: "ri-github-fill",
    tone: "text-zinc-200 border-zinc-500/50 hover:bg-zinc-800/80",
  },
  {
    label: "Instagram",
    value: "@cgtheavenly",
    href: "https://instagram.com/cgtheavenly",
    icon: "ri-instagram-fill",
    tone: "text-pink-200 border-pink-500/40 hover:bg-pink-500/10",
  },
];
  ...
  return (
    <>
      ...
      <section className="tentang mt-28 py-10" id="tentang">
        ...
        <div className="timeline mt-28">
          <div className="mb-14 grid gap-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
            <div data-aos="fade-up" data-aos-duration="1000">
              <span className="mb-3 inline-flex rounded-full border border-emerald-400/30 bg-emerald-400/10 px-3 py-1 text-sm font-semibold text-emerald-200">Career Journey</span>
              <h2 className="mb-4 text-4xl font-bold leading-snug text-zinc-100">Timeline Perjalanan</h2>
              <p className="text-base leading-loose text-zinc-400">
                Perjalanan saya berkembang dari ketertarikan pada visual digital, lalu masuk ke pengembangan web, sistem informasi, UI/UX, sampai aplikasi berbasis data.
              </p>
            </div>
            <div className="rounded-xl border border-zinc-700/40 bg-zinc-800/50 p-5" data-aos="fade-up" data-aos-duration="1000" data-aos-delay="200">
              <p className="text-sm font-semibold uppercase tracking-wide text-violet-200">Arah saat ini</p>
              <p className="mt-2 text-base leading-loose text-zinc-300">
                Saya sedang menyiapkan diri untuk peluang magang atau kolaborasi sebagai developer yang mampu menjembatani kebutuhan pengguna, tampilan antarmuka, dan logika sistem.
              </p>
              <a href="/assets/CV_Muhammad Dava Kayla Kalam Perdana.pdf" download className="mt-4 inline-flex w-fit items-center justify-center gap-2 rounded-lg border border-violet-500/50 px-4 py-2 font-semibold text-violet-200 transition-colors hover:bg-violet-500/10">
                CV Lengkap <i className="ri-file-download-line ri-lg"></i>
              </a>
            </div>
          </div>

          <div className="relative grid gap-6 lg:gap-0">
            <div className="absolute left-1/2 top-8 hidden h-[calc(100%-4rem)] w-px -translate-x-1/2 bg-gradient-to-b from-violet-500/0 via-violet-500/40 to-emerald-400/0 lg:block"></div>
            {listTimeline.map((item) => (
              <article key={item.id} className={`relative grid gap-4 lg:grid-cols-2 lg:gap-12 ${item.id % 2 === 0 ? "lg:[&>div]:col-start-2" : ""}`} data-aos="fade-up" data-aos-duration="1000" data-aos-delay={item.id * 120}>
                <div className="group relative rounded-xl border border-zinc-700/40 bg-zinc-800/70 p-5 shadow-md transition-colors hover:border-violet-500/50 hover:bg-zinc-800/90">
                  <span className={`absolute top-6 hidden h-4 w-4 rounded-full border border-violet-300/60 bg-zinc-900 shadow-[0_0_0_6px_rgba(124,58,237,0.12)] lg:block ${item.id % 2 === 0 ? "-left-[3.25rem]" : "-right-[3.25rem]"}`}></span>
                  <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      <span className="flex h-11 w-11 items-center justify-center rounded-lg border border-violet-500/40 bg-violet-500/10 text-violet-200">
                        <i className={`${item.icon} ri-xl`}></i>
                      </span>
                      <span className="rounded-full border border-zinc-600/70 bg-zinc-900/70 px-3 py-1 text-sm font-semibold text-zinc-300">{item.periode}</span>
                    </div>
                    <span className="text-sm font-bold text-violet-300">0{item.id}</span>
                  </div>
                  <p className="mb-2 text-sm font-semibold uppercase tracking-wide text-emerald-200">{item.tempat}</p>
                  <h3 className="mb-3 text-2xl font-bold leading-snug text-zinc-100">{item.title}</h3>
                  <p className="mb-5 text-base leading-loose text-zinc-300">{item.desk}</p>
                  <div className="grid gap-3 border-t border-zinc-700/50 pt-4">
                    <p className="text-sm leading-loose text-zinc-400">
                      <span className="font-semibold text-zinc-200">Hasil tahap ini:</span> {item.impact}
                    </p>
                    <span className="w-fit rounded-md border border-violet-500/30 bg-violet-500/10 px-3 py-1 text-sm font-semibold text-violet-200">{item.highlight}</span>
                  </div>
                </div>
              </article>
            ))}
        ...
        <div className="tools mt-28">
          ...
          <p className="w-full text-base leading-loose text-zinc-400 sm:w-3/4 md:w-2/3 lg:w-3/4 xl:w-2/5" data-aos="fade-up" data-aos-duration="1000" data-aos-delay="300">
            Beberapa tools dan teknologi yang biasa saya gunakan dalam membuat proyek.
          </p>
      ...
      <section className="kontak mt-28 p-0 sm:p-10" id="kontak">
        ...
        <p className="mx-auto mb-10 max-w-2xl text-center text-base leading-loose text-zinc-400" data-aos="fade-up" data-aos-duration="1000" data-aos-delay="300">
          Terbuka untuk peluang magang, kolaborasi proyek, undangan interview, dan diskusi profesional terkait web development, UI/UX, maupun sistem berbasis data.
        </p>

        <div className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="grid content-start gap-3" data-aos="fade-up" data-aos-duration="1000" data-aos-delay="400">
            {contactMethods.map((method) => (
              <a key={method.label} href={method.href} target={method.href.startsWith("http") ? "_blank" : undefined} rel={method.href.startsWith("http") ? "noreferrer" : undefined} className={`flex items-center gap-4 rounded-xl border bg-zinc-800/60 p-4 transition-colors ${method.tone}`}>
                <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-zinc-900/80">
                  <i className={`${method.icon} ri-xl`}></i>
                </span>
                <span>
                  <span className="block text-sm font-semibold uppercase tracking-wide text-zinc-400">{method.label}</span>
                  <span className="block break-all font-semibold">{method.value}</span>
                </span>
              </a>
            ))}
          </div>

          <form onSubmit={handleContactSubmit} className="w-full rounded-xl border border-zinc-700/40 bg-gradient-to-br from-zinc-800/90 to-violet-900/80 p-6 shadow-md sm:p-8" autoComplete="off" data-aos="fade-up" data-aos-duration="1000" data-aos-delay="500">
            <div className="flex flex-col gap-6">
              <p className="text-sm leading-loose text-zinc-300">Mohon isi form berikut untuk membuat draft email otomatis dengan informasi yang sudah rapi.</p>
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="flex flex-col gap-2">
                  <label className="font-semibold text-zinc-200">Nama Lengkap</label>
                  <input type="text" name="nama" placeholder="Masukkan nama..." className="rounded-md border border-zinc-700/60 bg-zinc-900/60 p-3 text-zinc-100 placeholder:text-zinc-400" required />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="font-semibold text-zinc-200">Email</label>
                  <input type="email" name="email" placeholder="Masukkan email..." className="rounded-md border border-zinc-700/60 bg-zinc-900/60 p-3 text-zinc-100 placeholder:text-zinc-400" required />
                </div>
              </div>
              <div className="flex flex-col gap-2">
                <label htmlFor="pesan" className="font-semibold text-zinc-200">Pesan</label>
                <textarea name="pesan" id="pesan" rows="7" placeholder="Tulis pesan..." className="rounded-md border border-zinc-700/60 bg-zinc-900/60 p-3 text-zinc-100 placeholder:text-zinc-400" required></textarea>
              </div>
              <div className="text-center">
                <button type="submit" className="inline-flex w-full cursor-pointer items-center justify-center gap-2 rounded-lg border border-violet-700/60 bg-violet-700/90 p-3 font-semibold text-white transition-colors hover:bg-violet-600/90">
                  Kirim Pesan <i className="ri-send-plane-line ri-lg"></i>
                </button>
              </div>
            </div>
          </form>
